...
https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE25030
...
```{r}
load(file.path(dataDir,"dataset2_eset.RData"))
```

```{r}
count.table <- read.table(file.path(dataDir,"dataset2_count_table.txt"), header=TRUE, sep="\t", row.names=1, quote="", comment.char="")
dim(count.table)
head(count.table)
